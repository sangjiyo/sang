...
最后完成了**lab-9**的全部测试，在**ai**的帮助下，我只花费了一个星期左右的时间，便完成了这个在课程设计上需要十七周才能完成的操作系统实验，虽然几乎所有代码都是不是我自己实现的，但是我通过整个实验过程，还是有许多收获的：

- 对操作系统的整个架构都更加熟悉了，这归功于九个实验的层层设计
- 对内存，进程，文件等组织形式更加了解了，因为每涉及到新的一层架构的实现，都要有对物理空间的申请，申请过后的管理，使用，到最后的释放
- 对汇编代码也不是那么一窍不通了，这得归功于之前的一次长达整整两天的不断调试，虽然方向一直都是错的，但是确实在不断读改汇编代码的过程中，发现汇编也不是那么晦涩难懂，至少比01好很多，不是么
- 还有很关键的c语言，因为整个操作系统几乎都是使用c语言完成的，特别是其中设计到的结构体的操作，链表指针的操作等，极为繁杂，常常需要像解线球一样，在重重的链表指针里，判断它们的走向

不过现在的这个操作系统虽然完成了，但是还是欠缺很多东西，比如很重要的**shell程序**——人机交互的桥梁，还有一些实用的程序等，这些可能会在以后慢慢完善。

## 十二、搭建人机交互的桥梁——shell

一个成熟的操作系统一定要能完成人机交互，所以**shell**的实现是必不可少的，**shell**其实有点相当于之前的**proczero**进程，它永远不会退出，不会结束，而后在**shell**中不断接受用户的输入，然后进行处理，如果用户输入了对应的命令和参数，解析后就交给对应的程序进行执行，这么看来，**shell**好像就充当一个永不停歇的中转站的作用，只不过在这个操作系统中，**ai**最终给我实现的**shell**也是一个由**proczero**调用的普通用户程序。
